    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Ticket Channels</li>
                </ol>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Ticket Channel</h4>
                        ...
                        <a href="" data-toggle="modal" data-target="#responsive-modal3" id="addLeadsource" type="button"
                            class="btn btn-outline-warning float-right" style="margin-right: 10px; font-size: 12px"><i class="ti-plus"></i>Add Channel</a>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item text-dark" type="button"
                                                        ...
                                                        id="{{$row->id}}" name="{{$row->channel_name}}"
                                                        href=""><i
                                                            class="ti-marker-alt" style="font-size: 12px"></i> Edit</a>
                                                    <a class="dropdown-item text-dark" onclick="deleteChannel(this)"
                                                        id="{{$row->id}}"
...
<div id="responsive-modal4" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h4 class="modal-title text-white" id="exampleModalLabel1">Update Ticket Channel</h4>
                <button type="button text-white" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
            <form id="ticketChannelUpdateForm">
                    @csrf
                    <input type="hidden" name="id" id="channelId">
                    <div class="form-group">
                        <label for="recipient-name" class="control-label">Channel Name</label>
                        <input type="text" name="channel_name" id="channelName" class="form-control" required>
                    </div>
            </div>
            <div class="modal-footer">
                <button type="submit" class="btn btn-sm btn-danger waves-effect waves-light"><i class="fa fa-check"></i>
...
@endsection
@push('tickect-channels-page-script')
{{-- channel --}}
<script>
...
<script>
      ...
      function editChannel(elem) {
        var id = $(elem).attr("id");
        var channel = $(elem).attr("name");
        $('#channelId').val(id);
...
<script>
    $(document).ready(function(){
       $('#ticketChannelUpdateForm').on('submit', function(event){
            event.preventDefault();
            $.ajax({
                url:'{{route("admin.ticket.channel.update")}}',
                method:'post',
                data:$(this).serialize(),
                dataType:'json',
                 success: function (data) {
...
<script>
         ...
         function deleteChannel(elem) {
        var id = $(elem).attr("id");
        $.ajax({
            url: "{{ route('admin.ticket.channel.delete') }}",
            method: "POST",
            dataType: "json",
